...
namespace Step_v0
{
    public partial class Form1 : Form
    {
        ...
        {
            XmlDocument Doc = new XmlDocument();
            Doc.Load("C:\\Users\\Serg\\Desktop\\Проект ST&P\\23.12\\Poezd-master\\Poezd-master\\28.11 ST&P\\Step_v0\\Step_v0\\Distanation.xml");
            XmlElement Root = Doc.DocumentElement;
            string start = "", end = "",id="";
            foreach (XmlNode node in Root)
            {
        ...
        private void Receive_data1_Click(object sender, EventArgs e)
        {
                ...
                if (str == "")
                {
                    ...
                    for (int i = 0; i < trains.Count; i++)
                    {
                        for (int j = 0; j < marshruti.Count; j++)
                        {
                            ...
                            {
                                trains[i].last_ostanovka(marshruti[j],realtime);
            ...
            if (f == 2)
            {
                string str1 = textBox4.Text, str2 = textBox3.Text;
                bool fa = false;Vivod.Rows.Add();
                    ...
                    for (int i = 0; i < marshruti.Count; i++)
                    {
                        if ((marshruti[i].Start+marshruti[i].End).Contains(str_ob))
                        {
                            for (int j = 0; j < trains.Count; j++)
                            {
                                ...
                                {
                                    trains[j].last_ostanovka(marshruti[i],realtime);
            ...
            if (f == 1)
            {
                string str1 = textBox1.Text;string str2 = textBox6.Text; bool fa = false; vivod_pas.Rows.Add();vivod_bil.Rows.Add();
                string str_ob = str1 + str2;
                if (str_ob == "")
                {
                    fa = true;
